...
require ("twitter_populate2.php");
		...
		$state_abbreviation = $_GET['stateAbbr'];
		$city = $_GET['cityName'];
		...
		$yql_query = "select latitude, longitude, name, description from upcoming.events where woeid in (select woeid from geo.places where text='".$city.$state_abbreviation."' limit 1)";
		//$yql_query = "select latitude,longitude,name,description from upcoming.events where woeid in (select woeid from geo.places where text='".$city.$state_abbreviation."' limit 1)";
		echo $yql_query;
		...
		if(!is_null($phpObj->query->results)){
			...
			foreach($phpObj->query->results->event as $event){
				...
				if(!$mysqli->query($sql_node)){
					printf("\nFailed to insert " . $lat . " " . $long . " into database". $mysqli->error."\n");
					var_dump($sql_idea);
				}
				unset($sql_node);
				unset($sql_idea);
